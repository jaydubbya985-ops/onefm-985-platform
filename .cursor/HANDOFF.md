...
| Live `#/ops` | **Still DEMO on production** (`index-BJ4yefZ1.js`). **PR #16 is ready for review**. LIVE via baked `VITE_*`, `window.__ONEFM_OPS__` snippet, or `/.netlify/functions/ops-config`. Drag-drop zip does not include functions — snippet or Git deploy is required for LIVE. Do not merge without **EXE**. |
