...
| Live `#/ops` | **Still DEMO on production** (`index-BJ4yefZ1.js`). **PR #16 is ready for review** (HEAD `5468293`). LIVE banner when Supabase is configured; DEMO seeds stay in DEMO only; FOOTT + Jason’s TV upserted on first live load. LIVE invoice create does not list the DEMO sponsor book. LIVE enquiry assign uses the signed-in email. Runtime `/.netlify/functions/ops-config` reads Netlify site env so `#/ops` can go LIVE **without** GitHub `VITE_*` secrets after this function is deployed. Do not merge without **EXE**. |
