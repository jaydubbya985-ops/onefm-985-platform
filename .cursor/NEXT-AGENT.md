...
Live `#/ops` is already **LIVE — not DEMO**. `npm run live` fails until **fm985.com.au** apex is verified. NEED JAY: SiteGround DNS — replace `resend._domainkey` TXT with Resend’s DKIM CNAME; add `include:amazonses.com` to SPF; do not change Outlook MX.
