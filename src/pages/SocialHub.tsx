...
const GUIDES = [
  { title: 'The ONE FM Voice', icon: <Mic size={40} />, color: 'text-one-gold', desc: 'Tone, language, and personality guidelines for all social content', pages: '12 pages' },
  { title: 'Optimal Posting Times', icon: <Clock size={40} />, color: 'text-data-teal', desc: 'Platform-specific timing recommendations based on audience data', pages: '8 pages' },
  { title: 'Hashtag Strategy', icon: <Hash size={40} />, color: 'text-data-violet', desc: 'Curated hashtag sets for maximum reach and engagement', pages: '6 pages' },
...
function HeroSection() {
  ...
  return (
    <section className="relative overflow-hidden" style={{ height: '50vh', minHeight: 400 }}>
      <GridPattern />
      <div className="relative z-10 flex flex-col justify-center h-full px-4 sm:px-6 max-w-[700px] mx-auto md:mx-0 md:ml-[10vw]">
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        <motion.p
          ...
          className="font-body text-one-white mb-8 max-w-lg"
        >
        ...
        >
          {stats.map((s, i) => (
            ...
            >
              <div className="font-stat text-one-gold" style={{ fontSize: '2.5rem' }}>{s.value}</div>
...
function AssetLibrary() {
  ...
  return (
    <section id="assets" className="bg-one-navy section-padding">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
            {BRAND_COLORS.map((color) => (
              <div key={color.hex} className="group cursor-pointer" onClick={() => copyHex(color.hex)}>
                <div
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="btn-secondary text-xs">Download Font Package</button>
          </div>
...
function TemplatesSection() {
  ...
  return (
    <section id="templates" className="bg-one-navy section-padding">
        ...
        >
          ...
          <div className="absolute inset-0 bg-one-navy/60 flex flex-col justify-end p-6 md:p-8">
            ...
            <div className="flex gap-3">
              <button className="btn-primary text-xs" onClick={(e) => { e.stopPropagation(); openModal(TEMPLATES[0]) }}>
                <Eye size={14} /> Preview
              </button>
              <button className="btn-secondary text-xs" onClick={(e) => e.stopPropagation()}>Download</button>
            </div>
          </div>
        </motion.div>
            ...
            {filteredTemplates.map((template, i) => (
              ...
              >
                <div className="relative h-[280px] overflow-hidden">
                  <img src={template.image} alt={template.name} className="w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-500" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <button className="btn-secondary text-xs">Open in Canva</button>
                  </div>
                </div>
              </div>
            </motion.div>
    ...
    <section className="bg-one-navy section-padding">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
              ...
              {allCells.map((day, i) => (
                <motion.div
                  ...
                  className={`relative bg-one-navy border border-one-border rounded-lg p-2 min-h-[100px] hover:bg-one-navy/80 transition-colors ${
                    day === new Date().getDate() ? 'ring-1 ring-amber' : ''
                  }`}
                >
                  {day && (
                    <>
        ...
        <motion.div
          ...
          className="glass-card mt-8 p-5 border-l-2 border-l-amber"
        >
  ...
  return (
    <section className="bg-one-navy section-padding">
      <div className="max-w-[1000px] mx-auto px-4 sm:px-6">
        ...
        >
          <h2 className="font-h2 text-one-white mb-2">POSTING TOOLKIT</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-10">
          {GUIDES.map((guide, i) => (
            <motion.div
              ...
              whileHover={{ y: -4 }}
              className="glass-card p-6 cursor-pointer group hover:border-one-gold/30 transition-colors"
            >
              ...
              <div className="flex items-center justify-between">
                ...
                <a href="#" className="font-label text-one-gold text-[10px] hover:text-one-gold transition-colors">Read Guide →</a>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-3">
          {['Caption Generator', 'Hashtag Picker', 'Image Resizer', 'Best Time Checker'].map((tool) => (
            <button key={tool} className="btn-secondary text-xs">{tool}</button>
          ))}
        </div>
      </div>
    </section>
...
function CaptionGenerator() {
  ...
  return (
    <section className="section-padding" style={{ background: 'linear-gradient(180deg, #1A1A1F, #0D0D0D)' }}>
      <div className="max-w-[800px] mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <h2 className="font-h2 text-one-white mb-2">AI CAPTION GENERATOR</h2>
              ...
              <div className="flex gap-2 flex-wrap">
                {['Hashtags', 'CTA', 'Emoji', 'Mentions'].map((opt) => (
                  <label key={opt} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-one-border font-label text-[10px] text-muted cursor-pointer hover:border-one-gold/50 transition-colors">
                    <input type="checkbox" defaultChecked className="accent-amber w-3 h-3" />
                    {opt}
                  </label>
                ))}
              </div>
